...
Consequently, researchers had to develop their\comment{---often simplified---}{ça sonne drole... je ne suis pas convaincu}own solutions, with little or no dissemination to the community, limiting knowledge and expertise sharing.
...
These derivatives can be obtained by \comment{finite differences}{Dire que c'est le cas habituellement/jusqu'à maintenant?} (inaccurate thus comprising convergence) or computed automatically using \comment{algorithmic differentiation}{Dire que ça nécessite que le code de base qui décrit la dynamique soit écrit en CasADi compatible?} [CasADi].


In order to promote the use of musculoskeletal optimal control among biomechanics researcher, we identified a strong need for a dedicated tool, as shown by the recently launched \comment{\moco}{Dans l'article, ils l'appelle OpenSim Moco, est-ce qu'on devrait se fier à ça?} [\addref, Opensim]. 
...
To develop such a software, four interrelated components are required: \comment{\textit{i)} a musculoskeletal modeling software, with a visualization module (multibody kinematics and dynamics, muscle dynamics, etc.), \textit{ii)} a method for automatic differentiation, \textit{iii)} a discretization approach, and \textit{iv)} a nonlinear programming (NLP) solver.}{Je suis d'accord avec ces points, mais je ne pense pas que ça soit évident pourquoi... Exemple, le ii, quelqu'un pourrait simplement dire : dérivez vous-même... ou le i pourquoi une vizualisation est nécessaire? Je ne suis pas sûr comment intégrer ces questions par contre...} 
