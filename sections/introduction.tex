...
The complexity of these systems has led scientists to formulate their simulations as optimal control problems (OCP), relying on efficient non-linear optimization software to find trajectories that fulfil a desired task while enforcing the system dynamics and minimizing a cost (e.g. \comment{time of execution}{Je n'arrive pas à dire pourquoi, mais ``time of execution'' m'a donné l'impression du temps d'optimisation, mais je n'arrive pas à trouver un meilleur mot}, energy expenditure, matching experimental data, etc.).
...
Its advantages are mostly the same as for direct collocation in addition to combine integration error control with fixed NLP dimensions, as it relies on \comment{possibly adaptive ODE solvers to integrate the system dynamics}{Dans ce contexte-ci, je suis d'accord [parce qu'on ne parle pas de différentiation algorithmique encore]}.
...
The OCP transcription uses direct multiple shooting to preserve the possibility of \comment{using adaptive ODE solvers in the integration}{Comme discuté, j'ai bien de la diffculté avec ça, je ne vois personnellement pas comment... Ceci dit, peut-être que ça se peut..}, which is fully parallelized for more efficiency.
