...
\begin{aligned}
\mathcal{J} = &~\underbrace{\omega_h h}_{\mathtt{MIN\_PREDICTED\_COM\_HEIGHT}}~+~\omega_{t}\sum^5_{i=1}
  ~\underbrace{(T_i - T_{i-1})}_{\mathtt{MIN\_TIME}}~+~\underbrace{\omega_{x} \| \mathbf{x}_+(T_5) - \mathbf{x}_+^*\|^2}_{\mathtt{TRACK\_STATE}}\\
  +~&\omega_{sd}\sum_{i=2}^4\int_{t=T_i}^{T_{i+1}}\underbrace{\left|\left|\frac{d\mathbf{\dot{q}}}{dt}\right|\right|^2}_{\mathtt{MIN\_STATE\_DERIVATIVE}}~dt
\end{aligned}  
$} 
\addtag  
