This example was designed to introduce \bioptim's ability to reduce the number of degrees-of-freedom (DoF) of a model via the \texttt{BidirectionalMapping} feature, to account for nonlinear boundaries on the controls, and to solve complex multiphase programs.
...
Since this is a full-body model, the root segment (i.e., the pelvis) was left \comment{uncontrolled}{MICK: unactuated. BEN: Pourquoi introduire une nouvelle nomenclature?}, reducing the number of control variables to 4, namely the shoulder, hip, knee and ankle flexions. 
...
Joint torques were bounded with \comment{nonlinear torque/angle/velocity relashionships}{MICK: ça c'est important est c'est caché. BEN: Oui, mais ça n'est pas un feature de bioptim. C'est une custom constraint... et je ne vois pas ce qu'on peut dire d'autre que ça en fait...} measured on a high level athlete using an isokinetic dynamometer. 
