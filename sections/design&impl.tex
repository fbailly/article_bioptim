...
Within this software \comment{bundle}{suite?}, \bioptim 's main role is to shape the problem in order to allow its dependencies to communicate efficiently, while providing an intuitive and flexible interface to the user (Fig.~\ref{fig:dependencies}).
...
The model file contains the geometrical characteristics, the segment inertias, the \comment{geometrical}{skin?} markers, the actuators of the model (muscles and joint torques accounting for angle/angular velocity/torque relationships) as well as bounds on joint kinematics and torques. 
...
\noindent More than 10 dynamics are implemented in \bioptim \footnote{\href{https://github.com/pyomeca/bioptim/blob/master/bioptim/dynamics/dynamics_functions.py}{github link}}, among which the controls can be muscle \comment{excitations/muscle activations/joint torques
}{Je trouve le mélange / et espace assez confus}, the states can be joint kinematics/muscle activations, including/excluding contacts, etc.
