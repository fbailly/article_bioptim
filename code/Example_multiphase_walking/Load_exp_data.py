...
class LoadData:
    ...
    def GetMarkers_Position(self):
        ...
        # LOAD C3D FILE
        measurements = c3d(self.c3d_file)
        ...
        markers = np.zeros((3, self.nb_markers, len(points[0, 0, :])))
    ...
    def Get_Q(self):
        ...
        """
        Q = np.loadtxt(self.Q_file)
        nb_frame = int(len(Q) / self.nb_q)
        q = np.zeros((self.nb_q, nb_frame))
    ...
    def GetForces(self):
        ...
        """
        measurements = c3d(self.c3d_file, extract_forceplat_data=True)
